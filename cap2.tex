...
\begin{itemize}
    ...
    Para el caso de la individualizaci\'on de filamentos, lo descrito en la ecuaci\'on \ref{eq:antProbabilities} corresponde a la probabilidad de elegir una arista de un conjunto de aristas vecinas (conjunto $N(s^{P})$) dadas la aristas que ya pertenecen al {\it tour} de la hormiga ($s^P$), mediante la heur\'istica miope (ecuaci\'on \ref{eq:heuristicaMiope}) que privilegia los candidatos que causen la menor desviaci\'on en la rectitud del camino. Cabe destacar que en el caso de que el conjunto $N(s^{P})$ este compuesto s\'olo por $c_{ij} \in ]\theta, 90]$, la ecuaci\'on \ref{eq:antProbabilities} se multiplica por $delta \in ]0,1]$, para agregar la opci\'on de no elegir ninguna arista con $\delta$ \% de probabilidad, quedando la heur\'istica miope como indica la ecuaci\'on \ref{eq:antProbInterQua}. El criterio de finalizaci\'on para la hormiga corresponde a que el conjunto $N(s^{P}) = \emptyset$.
...
En el paso de inicializaci\'on de ACO se deben definir valores para diversos par\'ametros, as\'i como margenes m\'aximos y m\'inimos para algunos. 

\begin{itemize}
    \item Par\'ametros de Feromonas: $\alpha$, valor inicial de $\tau_{ij}$, m\'argenes de $\tau_{ij}$: $\tau_{min}$ y $\tau_{max}$ y \\ {\it MAX\_AXIAL\_DISPLACEMENT}
    \item Par\'ametros de Heur\'istica: $\beta$, $\delta$ y {\it MAX\_SCORE}
\end{itemize}

Un aspecto de la ecuaci\'on \ref{eq:antProbabilities}/\ref{eq:antProbInterQua} que corresponde a la inicializaci\'on son los par\'ametros $\alpha$ y $\beta$ que representan la ponderaci\'on de la importancia entre las feromonas y la heur\'istica para el c\'alculo de probabilidad $P(c_{ij} | s^{P})$. A su vez, los par\'ametros iniciales correspondientes a las feromonas son el valor inicial de $\tau_{ij}$
